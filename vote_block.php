<?php
    ...
    $total1 = "select * from votes where position = 'block'";
    $runtotal = mysqli_query($con,$total1);
    $total = mysqli_num_rows($runtotal);
    
    //BJP COUNT Block

    $bjp1 = "select * from votes where  party = 'bjp' and position = 'block'";
    $count = mysqli_query($con,$bjp1);
    $bjpr = mysqli_num_rows($count);
    
    //INC COUNT Block

    $inc1 = "select * from votes where party = 'inc' and position = 'block'";
    $count1 = mysqli_query($con,$inc1);
    $incr = mysqli_num_rows($count1);

    //BSP COUNT Block

    $bsp1 = "select * from votes where  party = 'bsp' and position = 'block'";
    $count2 = mysqli_query($con,$bsp1);
    $bspr = mysqli_num_rows($count2);

    //AAP COUNT Block

    $aap1 = "select * from votes where  party = 'aap' and position = 'block'";
    $count3 = mysqli_query($con,$aap1);
    $aapr = mysqli_num_rows($count3);

    //INLD COUNT Block

    $inld1 = "select * from votes where party = 'inld' and position = 'block'";
...
<body>
  ...
  <div class="form">
      ...
      <ul class="tab-group" style="margin-top: 35px;">
		...
		<li class="tab active"><a href="#blk">Vote For BLOCK SAMITI</a></li>
    
     </ul>
      
      <div class="tab-content">

        <div id="blk" style="display: block;">
          ...
          <form method="post" enctype="multipart/form-data">
           ...
           <table border="1px" id="block" style="width: 550px; color: white; text-align: center;">
              <tr>
              <td colspan="5" style="text-align:center;background-color:#2962FF;font-size:25px;">Total Count of Block Votes is = <?php echo $total ?> </td>
              </tr>
