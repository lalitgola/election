<?php
    ...
    $total1 = "select * from votes where position = 'mla'";
    $runtotal = mysqli_query($con,$total1);
    $total = mysqli_num_rows($runtotal);
    
    //BJP COUNT

    $bjp1 = "select * from votes where party = 'bjp' and position = 'mla'";
    $count = mysqli_query($con,$bjp1);
    $bjpr = mysqli_num_rows($count);
    
    //INC COUNT

    $inc1 = "select * from votes where party = 'inc' and position = 'mla'";
    $count1 = mysqli_query($con,$inc1);
    $incr = mysqli_num_rows($count1);

    //BSP COUNT

    $bsp1 = "select * from votes where party = 'bsp' and position = 'mla'";
    $count2 = mysqli_query($con,$bsp1);
    $bspr = mysqli_num_rows($count2);

    //AAP COUNT

    $aap1 = "select * from votes where party = 'aap' and position = 'mla'";
    $count3 = mysqli_query($con,$aap1);
    $aapr = mysqli_num_rows($count3);

    //INLD COUNT

    $inld1 = "select * from votes where party = 'inld' and position = 'mla'";
...
<body>
  ...
  <div class="form">
      ...
      <ul class="tab-group" style="margin-top: 35px;">
		...
		<li class="tab active"><a href="#mla">Vote For MLA</a></li>
    
     </ul>
      
      <div class="tab-content">

        <div id="mla" style="display: block;">


          <form method="post" enctype="multipart/form-data">

            <table border="1px" id="mla" style="width: 550px; color: white; text-align: center;">
              <tr>
              <td colspan="5" style="text-align:center;background-color:#2962FF;font-size:25px;">Total Count of MLA Votes is = <?php echo $total ?> </td>
              </tr>
