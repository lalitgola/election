<?php
  ...
  include "dbcon.php";
    ...
    $total1 = "select * from votes where position = 'dc'";
    $runtotal = mysqli_query($con,$total1);
    $total = mysqli_num_rows($runtotal);
    
    //BJP COUNT DC

    $bjp1 = "select * from votes where party = 'bjp' and position = 'dc'";
    $count = mysqli_query($con,$bjp1);
    $bjpr = mysqli_num_rows($count);
    
    //INC COUNT DC

    $inc1 = "select * from votes where party = 'inc' and position = 'dc'";
    $count1 = mysqli_query($con,$inc1);
    $incr = mysqli_num_rows($count1);

    //BSP COUNT DC

    $bsp1 = "select * from votes where party = 'bsp' and position = 'dc'";
    $count2 = mysqli_query($con,$bsp1);
    $bspr = mysqli_num_rows($count2);

    //AAP COUNT DC

    $aap1 = "select * from votes where party = 'aap' and position = 'dc'";
    $count3 = mysqli_query($con,$aap1);
    $aapr = mysqli_num_rows($count3);

    //INLD COUNT DC

    $inld1 = "select * from votes where party = 'inld' and position = 'dc'";
...
<body>
  ...
  <div class="form">
      ...
      <ul class="tab-group" style="margin-top: 35px;">
		...
		<li class="tab active"><a href="#dc">Vote For DC</a></li>
      ...
      <div class="tab-content">
        ...
        <div id="dc" style="display: block;">
          ...
          <form method="post" enctype="multipart/form-data">
             ...
             <table border="1px" id="dc" style="width: 550px; color: white; text-align: center;">
              <tr>
              <td colspan="5" style="text-align:center;background-color:#2962FF;font-size:25px;">Total Count of DC Votes is = <?php echo $total ?> </td>
              </tr>
